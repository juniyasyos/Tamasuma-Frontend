<template>
  <v-navigation-drawer app width="100px" :color="this.$vuetify.theme.dark ? '#252726' : '#F5F8FC'" class="text-center"
    v-model="eventDrawer" floating clipped-left>
    <v-avatar class="my-4" :color="this.$vuetify.theme.dark ? '#999999' : '#F5F8FC'">
      <v-img alt="Community Logo" class="shrink google-font" contain :src="require('@/assets/img/icon-tamasuma.png')"
        transition="scale-transition" width="45" />
    </v-avatar>
    <v-list>
      <v-btn depressed class="px-0 py-8 mb-2" style="border-radius: 16px" v-for="(link, i) in links" :key="i"
        ...
        <div>
          ...
          <span style="
              ...
              font-weight: 600;
            " class="mb-0 mt-4 google-font">{{ link.text }}</span>
        </div>
      </v-btn>
    </v-list>
