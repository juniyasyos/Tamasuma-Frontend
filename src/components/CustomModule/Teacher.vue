<template>
  <v-dialog v-model="dialog" width="700">
    <template v-slot:activator="{ on }">
      <div v-on="on" style="cursor: pointer;height:100%" class="text-center py-4 ma-1 py-3"
        :class="$vuetify.theme.dark ? 'aura-card-dark' : 'aura-card-light'">
        <v-avatar size="100">
          <img :src="getImgUrl(data.image, 'profile.jpg')" :lazy-src="getImgUrl(data.image, 'profile.jpg')" />
        </v-avatar>
        <p class="mt-3 mb-0 google-font mb-0" style="font-size:105%">{{ data.name }}</p>
        <p class="mt-1 mb-0 google-font mt-0" style="font-size:80%">{{ summeryText(data.company.name, 20) }}</p>
      </div>
    </template>
